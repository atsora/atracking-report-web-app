...
<input type="hidden" id="widget" value="HIDDEN" />
...
<c:choose>
  <c:when test="${empty scalarParameter.value}">
    <input type="hidden" id="${scalarParameter.name}_value" value="" />
  </c:when>
  <c:otherwise>
    <input type="hidden" id="${scalarParameter.name}_value" value="${scalarParameter.value[0]}" />
  </c:otherwise>
</c:choose>